...
**retryStrategy:** `Optional<RetryStrategy>` — Any retry strategy for task execution or update.
...
**sinceTimestamp:** `Optional<OffsetDateTime>` 
...
**pageToken:** `Optional<String>` — Opaque cursor for continuing the same list request. Start a new listing without the previous cursor if any query parameter or listing scope changes.
...
**allObjectsInMesh:** `Optional<Boolean>` — Lists objects across all environment nodes in a Lattice Mesh. When false or unset, only objects held by the local node are returned.
...
**ingressId:** `Optional<String>` — Identifier of the live ingress stream to re-publish as an egress stream.
...
**mpegTs:** `Optional<Map<String, Object>>` 
...
**rtsp:** `Optional<RtspSettings>` — Pull from a caller-supplied RTSP URL.
